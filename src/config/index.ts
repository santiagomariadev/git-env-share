...
function resolveEncryptionKey(
  rawEncryptionKey?: unknown,
  fallback: 'age' | 'ssh' = DEFAULT_CONFIG.encryptionKey,
): 'age' | 'ssh' {
  const normalizedKey =
    typeof rawEncryptionKey === 'string' ? rawEncryptionKey.toLowerCase() : undefined;

  if (
    normalizedKey &&
    VALID_ENCRYPTION_KEYS.includes(normalizedKey as (typeof VALID_ENCRYPTION_KEYS)[number])
  ) {
...
export function hasExplicitConfig(projectRoot = process.cwd()): boolean {
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    if (
      ...
      Object.keys(packageConfig).length > 0
    ) {
      return true;
    }
  }
...
export function loadGitEnvShareConfig(
  projectRoot = process.cwd(),
  overrides: Partial<GitEnvShareConfig> = {},
): GitEnvShareConfig {
  const rootDir = projectRoot || process.cwd();
  const packageJsonPath = path.join(rootDir, 'package.json');
  const configFilePath = path.join(rootDir, '.git-env-share.config');
  ...
  config.githubUsernames = normalizeArray(
    config.githubUsernames || config.githubUsers || config.githubUser || [],
  );
  ...
  const rawTrigger = String(
    config.encryptionTrigger || DEFAULT_CONFIG.encryptionTrigger,
  ).toLowerCase();
  config.encryptionTrigger = VALID_ENCRYPTION_TRIGGERS.includes(
    rawTrigger as (typeof VALID_ENCRYPTION_TRIGGERS)[number],
  )
    ? rawTrigger
...
export function describeGitEnvShareConfig(config: Partial<GitEnvShareConfig> = {}): string {
  ...
  const triggerLabel =
    resolved.encryptionTrigger === ENCRYPTION_TRIGGERS.MANUAL ? 'manual' : 'commit';
...
export function resolvePrivateKeyPath(
  config: Partial<GitEnvShareConfig> = {},
  projectRoot = process.cwd(),
): string | null {
  ...
  const candidate =
    encryptionKey === ENCRYPTION_KEYS.SSH
