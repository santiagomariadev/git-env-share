...
export function isSshPublicKey(value: string | undefined | null): boolean {
  ...
  return /^(ssh-(rsa|ed25519|dss)|ecdsa-[A-Za-z0-9-]+|sk-(ssh-ed25519|ecdsa-sha2-nistp256)@openssh\.com)/.test(
    normalized,
  );
}

export function addSshRecipient(
  publicKey: string,
  options: { recipientsPath?: string } = {},
): string[] {
  ...
  if (!isSshPublicKey(normalizedKey)) {
    throw new Error(
      'Invalid SSH public key format. Expected ssh-..., ecdsa-..., or sk-... recipient.',
    );
  }
...
export function getSshHostFromRemote(): string {
  try {
    const remoteOutput = spawnSync('git', ['config', '--get', 'remote.origin.url'], {
      encoding: 'utf-8',
    });
...
export function resolveSshKeyPath(projectRoot = process.cwd()): string | null {
  ...
  try {
    const sshOutput = spawnSync('ssh', ['-G', host], {
      encoding: 'utf-8',
...
export function fetchGitHubPublicKeys(
  username: string,
...
export async function syncGitHubRecipientsFromConfig(
...
export async function addGitHubUser(
  username: string,
  options: { recipientsPath?: string; baseUrl?: string } = {},
): Promise<string[]> {
  const recipientsPath = options.recipientsPath || path.join(process.cwd(), '.agerecipients');
  const normalizedUsername = String(username || '').trim();
